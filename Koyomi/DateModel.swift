...
final class DateModel: NSObject {
    ...
    fileprivate var selectedDates: [Date: UIColor?] = [:]
    ...
    func willSelectDate(at indexPath: IndexPath) -> Date? {
        ...
        return selectedDates[date] != nil ? nil : date
    }
    
    // Select date in programmatically
    func select(from fromDate: Date, to toDate: Date?,color:UIColor) {
        if let toDate = toDate?.formated() {
            set(color, withFrom: fromDate, to: toDate)
        } else if let fromDate = fromDate.formated() {
            selectedDates[fromDate] = color
        }
    }
    ...
    func unselect(from fromDate: Date, to toDate: Date?) {
        if let toDate = toDate?.formated() {
            set(nil, withFrom: fromDate, to: toDate)
        } else if let fromDate = fromDate.formated() {
            selectedDates[fromDate] = nil
        }
    }
    
    func unselectAll() {
        selectedDates.keys.forEach { selectedDates[$0] = nil }
    }
    
    func select(with indexPath: IndexPath,color:UIColor) {
        ...
        case .single:
            selectedDates.forEach { [weak self] date, isSelected in
                ...
                if selectedDate == date {
                    me.selectedDates[date] = me.selectedDates[date] != nil ? nil : color
                } else if isSelected != nil {
                    me.selectedDates[date] = nil
                }
            }
            
        case .multiple:
            selectedDates[date(at: indexPath)] = selectedDates[date(at: indexPath)] != nil ? nil : color
        ...
        case .sequence:
            ...
            // user select selected date
                ...
                selectedDates[selectedDate] = nil
    ...
    func isSelect(with indexPath: IndexPath) -> UIColor? {
        let date = currentDates[indexPath.row]
        return selectedDates[date] ?? nil
    }
...
//        selectedDateKeys.forEach { selectedDates[$0] = true }
    ...
    func set(_ isSelected: UIColor?, withFrom fromDate: Date, to toDate: Date) {
        currentDates
