...
public enum KoyomiStyle {
    ...
    // Basic color
    case monotone, standard, red, orange, yellow, tealBlue, blue, purple, green, pink , igarden
    // Deep color
    ...
    var colors: Koyomi.Colors {
        ...
        case .igarden: return .init(dayBackgrond: .clear, weekBackgrond: .clear, week: .orange, weekday: .white, holiday: (saturday: .white, sunday: .white), otherMonth: .lightGray, separator: .clear)
...
//            backgroundColor = style.colors.separator
    ...
    @IBInspectable public var sectionSeparatorColor: UIColor = .clear { //UIColor.KoyomiColor.lightGray {
        didSet {
            sectionSeparator.backgroundColor = sectionSeparatorColor
        }
    }
    @IBInspectable public var separatorColor: UIColor = .clear { //UIColor.KoyomiColor.lightGray {
        didSet {
    ...
    @IBInspectable public var dayBackgrondColor: UIColor  = .clear
    @IBInspectable public var weekBackgrondColor: UIColor = .clear
    ...
    @discardableResult
    public func select(date: Date, to toDate: Date? = nil,color:UIColor) -> Self {
        model.select(from: date, to: toDate, color: color)
        return self
    }
    
    @discardableResult
    public func select(dates: [Date],color:UIColor) -> Self {
        dates.forEach { [weak self] date in self?.select(date: date,color:color) }
        return self
    }
    ...
    func configure(_ cell: KoyomiCell, at indexPath: IndexPath) {
        ...
        if indexPath.section == 0 {
            ...
            style = {
                ...
                switch (selectionMode, isSelected != nil) {
...
extension Koyomi: UICollectionViewDelegate {
    public func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        model.select(with: indexPath,color:curentColor)
        reloadData()
